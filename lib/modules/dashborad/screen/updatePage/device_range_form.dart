...
class Device_range_form extends StatefulWidget {
  final Devices devices;

  Device_range_form({@required this.devices});
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<Device_range_form> {
  ...
  void initState() {
    if (widget.devices != null) {
      ...
      _taskmin = widget.devices.min;
      _taskmax = widget.devices.max;
    }
              ...
              children: <Widget>[
                ...
                Row(
                  children: <Widget>[
                    new Flexible(
                      child: new TextField(
                          controller: TextEditingController()..text = _taskmin.toString(),
                          decoration: const InputDecoration(helperText: "Min"),
                          style: Theme.of(context).textTheme.body1,
                          keyboardType: TextInputType.text,
                          onChanged: (value) async {
                            await _dbHelper.updateDevicesMin(_taskId, double.parse(value));
                          }
                      ),
                    ),
                    SizedBox(width: 40,),
                    new Flexible(
                      child: new TextField(
                          controller: TextEditingController()..text = _taskmax.toString(),
                          decoration: const InputDecoration(helperText: "Max"),
                          style: Theme.of(context).textTheme.body1,
                          keyboardType: TextInputType.text,
                          onChanged: (value) async {
                            await _dbHelper.updateDevicesMax(_taskId, double.parse(value));
                          }
                      ),
                    ),
                  ],
